                              ...
                              QString &error)
{
    ...
    if ((mode & QIODevice::Append) != 0) {
        error = "QIODevice::Append is not supported for GZIP";
        return false;
    }
    if ((mode & QIODevice::ReadOnly) != 0
            && (mode & QIODevice::WriteOnly) != 0) {
        error = "Opening gzip for both reading and writing is not supported";
        return false;
        ...
        error = "You can open a gzip either for reading or for writing. Which is it?";
        return false;
    }
